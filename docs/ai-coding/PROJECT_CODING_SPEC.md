...
- 当前项目确实需要消息服务专属示例时，新增 `docs/ai-coding/project-examples` 或 `docs/ai-coding/business-examples`，不要放进公共 `examples`。
...
- 路径使用复数名词，例如 `/message/manage/templates`、`/message/manage/records`。
- 不使用动词路径表达标准 CRUD，例如 `/save`、`/update`、`/remove`、`/select`、`/page`。
- 查询使用 `GET`，分页通过集合资源表达，例如 `GET /message/manage/templates?tenantId=100&current=1&size=10`。
...
- 修改和删除时，资源 ID 放在 path 里，例如 `PUT /message/manage/templates/{id}`、`DELETE /message/manage/templates/{id}`。
- 写入 body 不重复表达 path 已经表达的动作；body 只承接资源字段和乐观锁 `version`。
- 当前租户优先从请求头、登录上下文或路径父资源中获取；不要在所有 body 中机械重复租户字段。
- 授权、绑定、发送、回调等关系动作按子资源表达，例如 `POST /message/manage/templates/{templateId}/test-messages` 创建模板测试消息。
- 无法自然映射为资源 CRUD 的业务动作，也要优先转成资源创建或子资源维护，例如发送消息使用 `POST /message/manage/messages` 创建发送请求。
...
- 公开方法和关键业务方法必须有 JavaDoc，说明用途、关键参数、返回值和消息/租户/权限副作用。
- 方法参数必须说明业务含义；不能只写“参数”或重复变量名。
- 认证授权、租户、权限、MQ、消息内容渲染、数据库乐观锁、SQL 参数校验、事务、异常处理、返回值组装等关键逻辑必须在逻辑块前说明“为什么这样做”和“守住什么边界”。
- 注释要解释业务目的、消息可见性、风险边界和框架衔接，不要只翻译代码或重复方法名。
...
需要数据权限的消息业务表必须走 `DataPermissionInterceptor` 的全局表规则，不要在 Service 查询方法里为每个页面手写数据范围过滤。没有负责人或部门归属语义的消息渠道、消息场景等配置表应放入忽略表或不声明表规则。
