...
4. 阅读 `AI_DESIGN_PATTERN_GUIDE.md`，确认消息服务适用的设计模式和禁止过度抽象规则。
5. 阅读 `PROJECT_CODING_SPEC.md`。
6. 阅读 `UTILS_PUBLIC_SPEC.md`，确认公共规范、错误码、数据库、乐观锁、分支流程和注释要求。
7. 如果任务涉及当前微服务已有业务、接口、权限码、默认数据或启动配置，阅读项目根目录 `README.md`。
8. 参考 `examples/` 下的示例。
9. 如涉及公共规范原文、错误码完整表、团队流程或 `utils` 能力，阅读 `archive/utils-markdown/` 下对应文档。
10. 判断本次代码是否属于公共工具类、通用组件、基础配置或跨微服务复用能力；如果是，先切到同级 `utils` 项目检索已有实现。
11. `utils` 已有能力时优先复用，不要在当前微服务重复编写；确实缺失时再在 `utils` 实现并安装依赖。
12. 只有明确属于当前微服务业务边界的代码，才按当前项目现有结构创建 Java 文件。
13. 新增或修改 Controller 时，先按 `PROJECT_CODING_SPEC.md` 的 RESTful 接口规范设计路径和 HTTP 方法。
14. 新增或修改 Controller 时，补齐 `@Tag`、`@Operation`、必要的 `@Parameter` 和请求/响应对象 `@Schema`，确保 Knife4j 页面展示业务名称而不是默认方法名。
15. 涉及 MySQL、Redis、RabbitMQ、Seata、XXL-JOB、Elasticsearch、Kibana、Zipkin 等基础设施地址时，优先读取项目根目录 `README.md` 的基础设施地址约定，不要在业务配置中新增裸 IP。
16. 新增或修改 README、AI 规范、配置、脚本、测试、示例和代码时，禁止写入个人电脑绝对路径；目录关系使用相对路径，外部安装位置使用环境变量或 `<PLACEHOLDER>` 占位符。
17. 代码编写完成后补充或更新 JUnit 5 测试用例；接口功能优先从 Controller 请求层覆盖请求参数、权限、统一响应和 Service 调用，再按风险补充 Service/Mapper 单元测试。
18. 检查项目根目录 `README.md`，已有内容时补充本次新增或调整的业务说明，缺少文件时新建。
19. 编译和测试验证，并按 `AI_ENGINEERING_GUARDRAILS.md` 输出风险、回滚和未验证项。

## 编码生成规则

- 业务编码必须由后端统一生成，不允许前端、脚本或测试数据各自拼接随机编码。
- 新增需要人工输入 `code` 的业务表单时，必须先检查同级 `utils` 或当前微服务是否已有可复用编码生成能力。
- 编码生成接口必须受当前业务管理权限保护，例如消息模板可使用 `message:template:create` 或更细的编码生成权限。
- 生成规则集中在当前业务服务类中，不要把编码格式散落在 Controller、前端或 SQL 脚本中。
- 生成目标必须覆盖当前微服务所有需要人工输入 `code` 的表单，例如消息模板、消息渠道、消息场景等。
...
安全细则单独维护在 `SECURITY_CODING_SPEC.md`。新增或修改接口、权限、消息内容、查询、文件、日志、上传下载、脱敏、测试时，必须先阅读该文件并按检查清单验证。
...
- 消息服务优先沿用 Controller、Service、Mapper、Entity、BO、Query、VO 分层。
- 消息渠道、模板类型、发送流程、收件箱状态、MQ 和通知通道出现稳定扩展点时，再使用 Strategy、Observer、State、Pipeline 或 Adapter。
- 接收侧和发送侧权限边界不能被抽象混淆。
