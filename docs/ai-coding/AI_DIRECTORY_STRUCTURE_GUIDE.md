...
本规范约束 AI 在 `message` 消息服务中新增、移动、拆分和命名目录的方式。目录管理必须基于当前 Java 17、Spring Boot、Gradle、MyBatis-Plus、RabbitMQ 和 Nacos 微服务结构。
...
| 目录 | 职责 |
| --- | --- |
| `com/kellen/message/controller` | 消息服务 HTTP 接口入口。 |
| `com/kellen/message/entity` | 消息、模板、发送记录等数据库实体和枚举。 |
| `com/kellen/message/entity/bo` | 新增、修改、发送等写入请求对象。 |
| `com/kellen/message/entity/query` | 查询请求对象。 |
| `com/kellen/message/entity/vo` | 接口响应视图对象。 |
| `com/kellen/message/mapper` | MyBatis-Plus Mapper 和 XML 对应接口。 |
| `com/kellen/message/service` | Service 接口。 |
| `com/kellen/message/service/impl` | Service 实现和发送/收件箱业务编排。 |
| `com/kellen/message/service/query` | 查询服务入参和复杂读模型。 |
| `com/kellen/message/service/results` | Service 层输出结果对象。 |
| `com/kellen/job` | 消息相关定时任务入口。 |
| `com/kellen/bean` | 当前服务本地基础对象；公共基础能力优先回到 `../utils`。 |

## 目录规则

- 新增消息能力优先放在 `com/kellen/message` 现有分层下，不新建并行的 `dao`、`domain`、`manager`、`modules` 体系。
- 发送渠道、模板类型、收件箱状态、MQ 处理等扩展应先复用现有 service / strategy / adapter 边界。
...
- 当前按技术分层组织包（controller/service/mapper/entity 等，package-by-layer）；当通知、模板、通道、投递记录等消息业务域在多个层目录中持续膨胀，且改动总是跨多个层目录联动时，才评估按业务特性分包（package-by-feature）。演进必须有真实维护痛点，不为小规模代码强行切换，并同步 Spring 组件扫描、MyBatis 扫描、测试和文档。
...
- 是否保持 `com/kellen/message` 消息服务边界清晰？
